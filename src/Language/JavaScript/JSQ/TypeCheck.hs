...
module Language.JavaScript.JSQ.TypeCheck where
...
instance JTypeCheck JVal where
    ...
    typecheck (JList xs) = typecheck (JObject $ M.fromList $ zip (map show [(0::Int)..]) xs)
                           -- fmap JTList . someUpperBound =<< mapM typecheck xs
    typecheck (JRegEx _) = undefined --regex object
    typecheck (JObject mp) = do
...
typecheckWithBlock :: (JsToDoc a, JSQ a, JTypeCheck a) => a -> TMonad JType

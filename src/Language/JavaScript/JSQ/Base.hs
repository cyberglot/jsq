...
newIdentSupply Nothing     = newIdentSupply (Just "jsqId")
...
data JSQGadt a where
    JSQGId   :: Ident -> JSQGadt Ident
    JSQGStat :: JStat -> JSQGadt JStat
    JSQGExpr :: JExpr -> JSQGadt JExpr
    JSQGVal  :: JVal  -> JSQGadt JVal
...
instance Compos JSQGadt where
    compos = jsqcompos

jsqcompos :: forall m c. (forall a. a -> m a) -> (forall a b. m (a -> b) -> m a -> m b) -> (forall a. JSQGadt a -> m (JSQGadt a)) -> JSQGadt c -> m (JSQGadt c)
jsqcompos ret app f' v =
    case v of
     JSQGId _ -> ret v
     JSQGStat v' -> ret JSQGStat `app` case v' of
     ...
     JSQGExpr v' -> ret JSQGExpr `app` case v' of
           ...
           UnsatExpr _ -> ret v'
     JSQGVal v' -> ret JSQGVal `app` case v' of
...
jsSaturate :: (JSQ a) => Maybe String -> a -> a
jsSaturate str x = evalState (runIdentSupply $ jsSaturate_ x) (newIdentSupply str)

jsSaturate_ :: (JSQ a) => a -> IdentSupply a
jsSaturate_ e = IS $ jfromGADT <$> go (jtoGADT e)
    where
      go :: forall a. JSQGadt a -> State [Ident] (JSQGadt a)
      go v = case v of
               JSQGStat (UnsatBlock us) -> go =<< (JSQGStat <$> runIdentSupply us)
               JSQGExpr (UnsatExpr  us) -> go =<< (JSQGExpr <$> runIdentSupply us)
               JSQGVal  (UnsatVal   us) -> go =<< (JSQGVal  <$> runIdentSupply us)
               _ -> composOpM go v
...
jsReplace_ :: JSQ a => [(Ident, Ident)] -> a -> a
jsReplace_ xs e = jfromGADT $ go (jtoGADT e)
    where
      go :: forall a. JSQGadt a -> JSQGadt a
      go v = case v of
                   JSQGId i -> maybe v JSQGId (M.lookup i mp)
                   _ -> composOp go v
      mp = M.fromList xs

--only works on fully saturated things
jsUnsat_ :: JSQ a => [Ident] -> a -> IdentSupply a
jsUnsat_ xs e = IS $ do
...
withHygiene ::  JSQ a => (a -> a) -> a -> a
withHygiene f x = jfromGADT $ case jtoGADT x of
    JSQGExpr z -> JSQGExpr $ UnsatExpr $ inScope z
    JSQGStat z -> JSQGStat $ UnsatBlock $ inScope z
    JSQGVal  z -> JSQGVal $ UnsatVal $ inScope z
...
withHygiene_ :: JSQ a => String -> (a -> a) -> a -> a
withHygiene_ un f x = jfromGADT $ case jtoGADT x of
    JSQGStat _ -> jtoGADT $ UnsatBlock (jsUnsat_ is' x'')
    JSQGExpr _ -> jtoGADT $ UnsatExpr (jsUnsat_ is' x'')
...
scopify x = evalState (jfromGADT <$> go (jtoGADT x)) (newIdentSupply Nothing)
    where go :: forall a. JSQGadt a -> State [Ident] (JSQGadt a)
          go v = case v of
                   (JSQGStat (BlockStat ss)) -> JSQGStat . BlockStat <$>
                                             blocks ss
                       where blocks [] = return []
                             ...
                             (<:>) = liftM2 (:)
                   (JSQGStat (ForInStat b (StrI i) e s)) -> get >>= \case
                          (newI:st) -> do
                             put st
                             rest <- jfromGADT <$> go (jtoGADT s)
                             return $ JSQGStat . ForInStat b newI e $ jsReplace_ [(StrI i, newI)] rest
                          _ -> error "scopify2"
                   (JSQGStat (TryStat s (StrI i) s1 s2)) -> get >>= \case
                          (newI:st) -> do
                            put st
                            t <- jfromGADT <$> go (jtoGADT s)
                            c <- jfromGADT <$> go (jtoGADT s1)
                            f <- jfromGADT <$> go (jtoGADT s2)
                            return . JSQGStat . TryStat t newI (jsReplace_ [(StrI i, newI)] c) $ f
                          _ -> error "scopify3"
                   (JSQGExpr (ValExpr (JFunc is s))) -> do
                            st <- get
                            let (newIs,newSt) = splitAt (length is) st
                            put (newSt)
                            rest <- jfromGADT <$> go (jtoGADT s)
                            return . JSQGExpr . ValExpr $ JFunc newIs $ (jsReplace_ $ zip is newIs) rest
                   _ -> composOpM go v
...
renderJs :: (JsToDoc a, JSQ a) => a -> Doc
...
-- | render(Prefix)Js.
renderPrefixJs :: (JsToDoc a, JSQ a) => String -> a -> Doc
renderPrefixJs pfx = jsToDoc . jsSaturate (Just $ "jsqId_"++pfx)
